Техническое задание на позицию Middle Backend-разработчика в компанию WinDI

Описание проекта

Разрабатываемый продукт — мессенджер с возможностью обмена сообщениями
и создания групповых чатов, аналогичный Telegram.
...
Тестовое задание
  Разработать мини-приложение, реализующее функционал чата с возможностью отправки сообщений,
  их хранения в базе данных и создания групповых чатов.
...
1. Чат (WebSocket)
  ...
  - Реализовать обработку статуса "прочитано" (отправитель получает уведомление,
...
1. Backend:
  x FastAPI для реализации REST API и WebSocket.
  x Асинхронная работа через asyncio.
  x PostgreSQL как основная база данных.
  x SQLAlchemy (использование ORM, асинхронные запросы).
  x Контейнеризация (Docker, Docker Compose).
  x Автоматическая документация (Swagger / OpenAPI).
...
Дополнительное задание по желанию
  ...
  x Добавить авторизацию через JWT (OAuth 2.0 / FastAPI Security).
  x Реализовать юнит-тесты с использованием Pytest.
